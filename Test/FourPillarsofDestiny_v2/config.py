...
class Settings:
    ...
    groups_root: Path = data_root / "groups"
    ...
    # 동적 나이 범위가 기본이다. 비활성화할 때만 아래 고정 범위를 쓴다.
    dynamic_age_range_enabled: bool = _env_bool(
        "DYNAMIC_AGE_RANGE_ENABLED",
        True,
    )
    max_younger_years: int = _env_int("MAX_YOUNGER_YEARS", 5)
    max_older_years: int = _env_int("MAX_OLDER_YEARS", 15)
    age_range_policy_version: str = "age-mode-gender-v1"
    ...
    # 포스텔러 적응형 검증
    # 로컬 상위 20명부터 조회하고, TOP 10이 안정될 때까지 5명씩
    # 추가하되 최대 50명까지만 확인한다.
    ai_top_n: int = 10
    adaptive_initial_count: int = _env_int(
        "ADAPTIVE_INITIAL_COUNT",
        20,
    )
    adaptive_step_count: int = _env_int(
        "ADAPTIVE_STEP_COUNT",
        5,
    )
    adaptive_max_count: int = _env_int(
        "ADAPTIVE_MAX_COUNT",
        50,
    )
    adaptive_stability_rounds: int = _env_int(
        "ADAPTIVE_STABILITY_ROUNDS",
        2,
    )
    adaptive_require_stability: bool = _env_bool(
        "ADAPTIVE_REQUIRE_STABILITY",
        False,
    )

    collector_version: str = "7.4.1-scoring-context-fix"
    parser_version: str = "8.4.1-natal-source-scoring-context-fix"
    scoring_version: str = "7.0-balanced-weighted-quality"
    run_schema_version: str = "7.2-personal-group-layout-age-policy"
    candidate_selection_version: str = (
        "dynamic-age-full-range-robust-time-v6"
    )
    ...
    ai_prompt_version: str = "top10-v13-hanja-readable"
    ai_schema_version: str = "top10-report-v13-hanja-readable"
    ...
    # 직접 입력한 여러 사람을 서로 비교하는 그룹 모드.
    group_max_members: int = _env_int(
        "GROUP_MAX_MEMBERS",
        20,
    )
    group_ai_max_members: int = _env_int(
        "GROUP_AI_MAX_MEMBERS",
        10,
    )
    group_ai_max_output_tokens: int = _env_int(
        "GROUP_AI_MAX_OUTPUT_TOKENS",
        20_000,
    )
    group_prompt_version: str = "group-v2-brief-reasons-hanja"
    group_schema_version: str = "group-report-v1"
